...
export default function BlogIndex() {
  ...
  return (
    <div className="min-h-screen bg-[#060913] text-gray-100 font-sans selection:bg-[#e0231c] selection:text-white relative">
      ...
      <nav className="bg-[#060913]/90 backdrop-blur-md border-b border-gray-800/60 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div>
            <Link href="/#contact" className="bg-[#111827] hover:bg-[#e0231c] border border-gray-800 hover:border-[#e0231c] text-white px-5 py-2 rounded-full text-xs font-bold transition shadow-sm">
              Hire Me
            </Link>
          </div>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-16 space-y-12">
        <header className="text-center space-y-4">
          <div className="inline-flex flex-col items-center">
            ...
            <div className="w-[25%] h-0.5 bg-[#e0231c] rounded-full"></div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-8 pt-4">
          {posts.map((post) => (
            <article key={post.id} className="bg-[#0d1322] border border-gray-800 p-8 rounded-3xl shadow-xl hover:border-[#e0231c]/50 transition group flex flex-col justify-between gap-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-wider">
                  <span className="text-[#ff4d4d] bg-[#e0231c]/10 border border-[#e0231c]/20 px-3 py-1 rounded-full">{post.category}</span>
                  <span className="text-gray-400 font-mono">{post.date}</span>
                </div>
                <h2 className="text-2xl font-bold text-white group-hover:text-[#ff4d4d] transition">
                  <Link href={`/blog/${post.slug}`}>{post.title}</Link>
                </h2>
                <p className="text-gray-300 leading-relaxed text-sm md:text-base">{post.excerpt}</p>
              </div>
              <div className="pt-2">
                <Link href={`/blog/${post.slug}`} className="inline-flex items-center gap-2 text-sm font-bold text-[#ff4d4d] hover:underline">
